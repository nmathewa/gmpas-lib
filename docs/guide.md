...
```bash
# on the compute node
gmpas view /scratch/run/ --host 0.0.0.0
...
- **gmpas never opens a browser for you.** One opening *on the compute node*
  would not be useful even if it worked, and what it usually does instead is
  hand the URL to a terminal browser that seizes the terminal the server is
  logging to. Pass `--browser` if you are on a machine where you do want one.
...
**Weight generation seems to hang** — gmpas streams ESMF's output as it
